  ...
  return (
    <div className="flex flex-col min-h-full bg-background">
      ...
      <div className="flex-1 overflow-y-auto">
        {activeTab === "today" && <TodayTab onStartJourney={onStartJourney} onViewOrders={() => setActiveTab("orders")} />}
...
/* ── Today tab ── */
function TodayTab({ onStartJourney, onViewOrders }: { onStartJourney: () => void; onViewOrders: () => void }) {
  return (
...
/* ── Orders tab ── */
function OrdersTab({ onStartJourney, onViewSchedule }: { onStartJourney: () => void; onViewSchedule: () => void }) {
  return (
    <div className="flex flex-col gap-3 px-4 pt-4 pb-8">
...
/* ── Schedule tab ── */
function ScheduleTab({ onStartJourney, onViewMap }: { onStartJourney: () => void; onViewMap: () => void }) {
  type Priority = "balanced" | "finish_early" | "leave_fast";
  const [priority, setPriority] = useState<Priority>("balanced");
  const [priorityOpen, setPriorityOpen] = useState(false);

  const priorityOptions: { id: Priority; label: string; desc: string; icon: React.ReactNode; time: string }[] = [
    {
      id: "balanced",
      label: "Cân bằng",
